...
% \subsection{Pseudocódigo do Método de Newton-Raphson}
% \begin{algorithm}[H]
%     \caption{Newton-Raphson para $f(x)=0$}
%     \begin{algorithmic}[1]
%         \Require chute inicial $x_0$, tolerância $\varepsilon$, máximo de iterações $N$
%         \For{$k = 0$ to $N$}
%             \State $x_{k+1} = x_k - \frac{f(x_k)}{f'(x_k)}$
%             \If{$|x_{k+1} - x_k| < \varepsilon$}
%                 \State \Return $x_{k+1}$
%             \EndIf
%         \EndFor
%         \State \Return \text{falha de convergência}
%     \end{algorithmic}
% \end{algorithm}
...
\begin{algorithm}[H]
    \caption{Método de Newton-Raphson (simplicado)}
    \label{alg:newtonraphson_simple}
    \begin{algorithmic}[1]
        \Require $x_0$ (chute), $\varepsilon$ (tolerância), $N$ (máx. iterações)
        \State $k \gets 0$
        \While{$k < N$}
