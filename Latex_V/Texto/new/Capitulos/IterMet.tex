...
Seja $f$ uma função contínua no intervalo $[a, b]$. Se $f(a)f(b) < 0$, então há pelo menos uma raiz $\xi \in (a,b)$. Se, além disso, existir $f'$ e $f'(x)$ preservar o sinal em $(a,b)$, então a raiz é única.
...
\end{itemize}
Além disso, a derivada de $f$ é $f'(x) = 3x^2 - 9$, cujas raízes são $\pm \sqrt{3}$, então nos intervalos (-$\infty$, -$\sqrt{3}$), (-$\sqrt{3}$, $\sqrt{3}$) e ($\sqrt{3}$, $\infty$) o sinal da derivada é preservado. Portanto, nos intervalos $(0, 1)$ e $(2, 3)$ há apenas uma raiz.
...
Outra forma de localizar raízes de uma dada função $f$ é escrevê-la como a diferença entre as funções $g$ e $h$, pois se $f(\xi) = 0$ temos que $g(\xi) - h(\xi) = 0$ ou, equivalentemente, $g(\xi) = h(\xi)$. Graficamente, $\xi$ é a abscissa do ponto de interseção entre as funções $g$ e $h$.
...
\FIXME{Completar}
\DanielP{gráficos estilo Vera} %ponto de acumulação, iterações vs. raiz desconhecida (por isso é numérico afinal). Já vale adiantar que no MPF (e por conseguinte em NR) $|varphi(x)| < 1$ {foda-se? não é f}
...
transforma o problema de buscar as raízes de uma função $f$ no problema de encontrar os pontos fixos de uma outra função $\varphi$, denominada de \textbf{função de iteração de ponto fixo}. 
A partir dessa função de iteração, uma sequência é construída recursivamente começando em um valor inicial $x_0$ que convergirá para a raiz $\xi$ de $f$, desde que sejam observadas certas condições sob a função $\varphi$ e o dado inicial $x_0$.

%Após localizar um intervalo que contenha uma raiz pelos métodos expostos,
O primeiro passo é gerar funções de iteração $\varphi$ para $f$, o que pode ser feito isolando $x$ na equação $f(x) = 0$. Por exemplo, manipulando a função $x^3 -9x + 3$  da seguinte forma
%nesse caso g(x) = x^3 - 8x + 3 e h(x) = x, sendo g(x) - h(x) = 0, temos pelo 2º método de localização de raízes que g(x) = h(x) então a abscissa do ponto de interseção entre g e h é a raiz da função original. Chamaremos eventualmente g(x) de varphi(x), e por essa manipulação g(x) = x, cumprindo a proposição 2.3.1.
\begin{equation*}
    x^3 - 8x + 3 = x 
\end{equation*} 
...
\begin{prop}
Seja $\xi$ uma raiz de uma função $f$ e seja $\varphi$ uma função de iteração associada a $f$. Então, $f(\xi) = 0$ se, e somente se, $\varphi(\xi) = \xi$.
\end{prop}
...
\begin{teo} \textbf{Convergência da Sequência Iterativa} \label{teoMPF}\\
    Seja $\xi$ uma raiz de f, isolada num intervalo I centrado nessa raiz. Considere uma função de iteração $\varphi$ associada a $f$. Sob as seguintes hipóteses:
    \begin{itemize}
        \item[i)] $\varphi$ e $\varphi'$ são contínuas em I,
...
Como $x_{k+1} = \varphi(x_k)$, subtraindo $\xi$ de ambos os lados da igualdade e usando o fato de que $\varphi(\xi)=\xi$ temos
    ...
    \end{equation}
    com $c_k$ entre $x_k$ e $\xi$. Então, substituindo (\ref{tvm}) em (\ref{difIt}) temos %, aplicando o módulo em ambos os lados da igualdade,
    \begin{align}\label{des}
        %x_{k+1} - \xi &= (x_k - \xi) \ \varphi'(c_k) \\
        |x_{k+1} - \xi| &= |(x_k - \xi) \ \varphi'(c_k)| \nonumber \\
        &= |x_k - \xi| \ |\varphi'(c_k)| \\
        &< |x_k - \xi| \nonumber
    \end{align} %pela hipótese ii
    uma vez que $|\varphi'(x)| < 1$. Como $x_0 \in I$, podemos concluir que $x_k \in I$ para todo $k$ já que, por (\ref{des}), $|x_k - \xi| < |x_0 - \xi|$. %parte mais importante da prova quiçá (que na real é o que vamos passar o resto da prova formalizando).
...
reais de funções não lineares, cuja ordem de convergência é pelo menos quadrática. A função de iteração específica para este método produz uma sequência em que cada termo $x_{k+1}$ corresponde, geometricamente, à \textbf{interseção da reta tangente} a $f$ no ponto $(x_k, f(x_k))$ com o eixo x. 
...
Dada uma função \(f\), o processo parte de uma estimativa inicial \(x_0\) e aplica a seguinte fórmula iterativa, a partir da escolha da derivada em (\ref{it})
\begin{equation} \label{nr}
    x_{k+1} = x_k - \frac{f(x_k)}{f'(x_k)},
\end{equation}
onde \(f'(x_k)\) é a derivada da função \(f\) avaliada em \(x_k\).
Para que o método convirja para a raiz correta, é necessário que \(f\) seja continuamente diferenciável em uma vizinhança da raiz e que \(f'(x) \neq 0\) nessa região. Além disso, a escolha adequada do ponto inicial \(x_0\) é crucial para garantir a convergência do método.
...
\begin{proof}
    ...
    \end{equation}
    Como $f'(\xi) \neq 0$ e $f'$ é contínua em I, é possível obter $I_1 \subset I$ tal que $f'(x) \neq 0$ no intervalo $I_1$. Assim, a função $f$ e suas derivadas primeira e segunda são contínuas em $I_1$ e, consequentemente, a função de iteração e sua derivada também.
    
     Uma vez que a $\varphi'$ é contínua em $I_1$ e $\varphi'(\xi) = 0$, é possível escolher $I_2 \subset I_1$ de modo que $|\varphi'(x)| < 1$ em $I_2$ tendo $\xi$ como centro do novo intervalo.
...
\end{equation}
O polinômio de Taylor de grau 2 para $f$ centrado em $x_k$ é
\begin{equation*}
