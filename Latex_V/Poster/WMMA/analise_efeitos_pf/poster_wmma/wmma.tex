...
\begin{minipage}[t]{0.31\textwidth}
  ...
  % figura 1
  \begin{center}
    \includegraphics{images/NumError1_mplt.pdf}
  \end{center}
  \parbox{24cm}{
    {
      \textbf{Figura 1:} Gráfico de $f(x)$ com precisão de 32 bits.
    }
  }
  \vspace{1cm}
  ...
  % % ERRO 2
  \parbox{24cm}{
    Já na Figura 2, consideramos a função $g(x) = \frac{(1+x)-1}{x}$, que matematicamente é igual a 1 para todo \(x \neq 0\). Contudo, ao calcular essa função utilizando ponto flutuante, para valores de $x$ muito pequenos, também observamos um comportamento oscilatório.
  % ERRO 1
  }
  % figura 4
  \begin{center}
    \includegraphics{images/NumError4_mplt}
  \end{center}

  \parbox{24cm}{
    {
      \textbf{Figura 2:} Gráfico de $g(x)$ com precisão de 64 bits.
    }
  }
...
\begin{minipage}[t]{0.31\textwidth}
  ...
  % figura 2
  \begin{center}
    \includegraphics{images/NumError2_mplt}
  \end{center}
  \parbox{24cm}{
    {
    \textbf{Figura 3:} Gráfico das curvas $p(x)$ e $q(x)$ com precisão de 64 bits.
    }
  }
  \vspace{1cm}
  ...
  Utilizando Diferenças finitas para aproximar a derivada de uma função $z(x)$ pelo quociente
  }
  {
  \[
    \textit{$D_z(x,h)$} = \frac{\textit{z}(x+h) - z(x-h)}{2h},
  \]
  }
  \parbox{24cm}{
  espera-se que, ao diminuir o valor de \(h\), a aproximação melhore. Mas devido aos erros de arredondamento em ponto flutuante, para h muito pequeno, o erro na aproximação começa a se comportar de forma caótica. A Figura 4 ilustra esse fenômeno para a função $z(x) = \frac{x^{3}}{3} - 3x + 3$ cuja derivada é dada por $z'(x) = x^{2} - 3$.
  }
  ...
  {
  \[
    \textit{E}(h) = |z'(1) - D_z(1,h)|.
  \]
  }    
  % figura 4
  \begin{center}
    \includegraphics{images/NumError3_mplt}
  \end{center}
  \parbox{24cm}{
    {
      \textbf{Figura 4:} Gráfico de $\textit{E}(h)$ com precisão de 32 bits.
    }
  }
