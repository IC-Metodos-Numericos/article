...
Um Sistema de Ponto Flutuante (SPF) é a técnica utilizada para aproximar números reais pelos computadores através de representações binárias compactas. O padrão definido pelo \cite{IEEE754} formaliza essa arquitetura, alocando bits para sinal, expoente e mantissa em precisões simples (32 bits) e dupla (64 bits), como representado na equação \ref{eq:spf}. A natureza discreta e finita do conjunto dos reais, essa abordagem introduz erros de representação intrínsecos que, ao se propagarem e acumularem em operações aritméticas sucessivas, podem comprometer severamente a exatidão numérica do sistema.
